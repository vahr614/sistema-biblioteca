...
# --- CONFIGURACIÓN DE SESIÓN (AUTO-LOGOUT 10 MIN) ---
...
# --- PANTALLA PRINCIPAL (ATENCIÓN) ---
@app.route('/', methods=['GET', 'POST'])
def atencion():
    error = None
    alumno_encontrado = None
    historial_encontrado = None
    
    if request.method == 'POST':
        # BUSCAR POR DNI (HISTORIAL)
        if 'btn_historial' in request.form:
            dni_busqueda = request.form['dni_historial'].strip()
            resultados = Alumno.query.filter_by(dni=dni_busqueda).filter(Alumno.nombre != None).order_by(Alumno.id.desc()).all()
            if resultados:
                historial_encontrado = resultados
                flash(f"✅ Se encontraron {len(resultados)} constancias.")
            else:
                error = "❌ No se encontraron constancias emitidas para este DNI."
    ...
    return render_template('atencion.html', error=error, alumno=alumno_encontrado, historial=historial_encontrado)
...
def login_admin():
    if request.method == 'POST':
        ...
        if admin and check_password_hash(admin.password, password_ingresado):
            ...
            session.permanent = True  # Activa timeout de 10 min
            ...
            registrar_log("LOGIN", "Inicio de sesión exitoso")
...
def admin_backup():
    ...
    registrar_log("BACKUP", "Descarga de copia de seguridad")
        ...
        if 'voucher_manual' in request.form:
            ...
            try:
                ...
                if m_float < 57.50:
                    ...
                    db.session.commit()
                    registrar_log("PAGO", f"Registró pago manual: {v}")
            ...
            if file.filename != '':
                try:
                    ...
                    db.session.commit()
                    registrar_log("CARGA MASIVA", f"Subió {count} pagos")
...
def admin_eliminar_pago(id):
    ...
    db.session.delete(p)
    db.session.commit()
    registrar_log("PAGO", f"Eliminó pago: {v}")
    return redirect(url_for('admin_pagos'))

# --- DEUDORES (CON LÓGICA DE NOMBRE COMBINADO) ---
...
def admin_deudores():
    ...
    if request.method == 'POST':
        ...
        facultad_txt = ""
        try:
            if request.form.get('facultad'):
                fac_obj = Facultad.query.get(int(request.form['facultad']))
                facultad_txt = fac_obj.nombre
        except: pass
        
        # La escuela ya viene como texto desde el formulario actualizado
        escuela_txt = request.form.get('escuela', '')
        ...
        if not Deudor.query.filter_by(identificador=identificador).first():
            db.session.add(Deudor(
                ...
                facultad=facultad_txt, 
                escuela=escuela_txt, 
                motivo=request.form['motivo']
            ))
            db.session.commit()
            registrar_log("BLOQUEO", f"Bloqueó a {nombre_completo}")
            flash(f"🚫 Bloqueado: {nombre_completo}")
    ...
    return render_template('admin_deudores.html', deudores=deudores, facultades=facs)
...
def admin_eliminar_deudor(id):
    ...
    db.session.delete(d)
    db.session.commit()
    registrar_log("DESBLOQUEO", f"Desbloqueó a {n}")
    flash("✅ Desbloqueado.")
    return redirect(url_for('admin_deudores'))
...
def admin_lista():
    ...
    query = Alumno.query.filter(Alumno.nombre != None)
    
    if busqueda:
        query = query.filter((Alumno.dni.like(f"%{busqueda}%")) | (Alumno.nombre.like(f"%{busqueda}%")))
    
    alumnos = query.order_by(Alumno.id.desc()).limit(500).all()
    return render_template('admin_lista.html', alumnos=alumnos, usuario=session.get('admin_user'), busqueda=busqueda)
...
def admin_exportar_excel():
    ...
    output.seek(0)
    registrar_log("EXPORTAR", "Descargó reporte Excel")
...
def admin_editar_tramite(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        registrar_log("EDICIÓN", f"Editó constancia de {alumno.nombre}")
...
def admin_usuarios():
    ...
    if request.method == 'POST':
        ...
        if not Administrador.query.filter_by(usuario=u).first():
            ...
            db.session.commit()
            registrar_log("USUARIO", f"Creó usuario {u}")
            flash(f"✅ Creado: {u}")
...
def admin_editar_usuario(id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        registrar_log("USUARIO", f"Editó usuario {admin_edit.usuario}")
        flash(f"✅ Actualizado.")
        return redirect(url_for('admin_usuarios'))
...
def admin_eliminar_usuario(id):
    ...
    if adm.usuario != session.get('admin_user'):
        ...
        db.session.commit()
        registrar_log("USUARIO", f"Eliminó usuario {u}")
    return redirect(url_for('admin_usuarios'))
...
# --- PDF GENERATOR (SOLUCIONADO EL ERROR DE RUTA) ---
...
def generar_pdf_sistema(id, download=False):
    ...
    fecha_texto = f"{datetime.now().day} de {meses[datetime.now().month-1]} de {datetime.now().year}"
