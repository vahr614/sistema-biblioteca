...
# Intentamos leer la variable de entorno (para la nube)
db_uri = os.environ.get('DATABASE_URL')

if not db_uri:
    # --- CONFIGURACIÓN LOCAL (TU PC) ---
    # ⚠️ CAMBIA '1234' POR LA CONTRASEÑA QUE PUSISTE AL INSTALAR POSTGRESQL
    # ⚠️ CAMBIA 'postgres' SI TU USUARIO ES OTRO
    db_uri = 'postgresql://postgres:biblioteca2025@localhost:5432/biblioteca_db'

# Ajuste para compatibilidad con la nube
...
# --- INICIALIZACIÓN DE LA BD (ESTA LÍNEA SOLO DEBE APARECER UNA VEZ) ---
db = SQLAlchemy(app)
        ...
        if 'voucher_manual' in request.form:
            ...
            try:
                ...
                if m_float < 57.50:
                    ...
                    db.session.commit()
                    flash(f"✅ Pago registrado: {v} (S/ {m_float})")
                else:
                    flash("⚠️ Ese voucher ya existe.")
            except:
        ...
        # 2. CARGA MASIVA
            ...
            if file.filename != '':
                try:
                    ...
                    db.session.commit()
                    flash(f"✅ Carga Masiva: {count} subidos. ({ignorados} ignorados)")
